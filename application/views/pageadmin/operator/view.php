<section class="content">
	...
	<div class="card">
		...
		<br>
		<div class="col-sm-2">
			<button href="#modalTambah" type="button" role="button" data-toggle="modal" class="btn btn-block btn-primary"><a class="ace-icon fa fa-plus bigger-120"></a> Add Operator</button>
		</div>
		<br>
...
<script type="text/javascript">
	if ($("#formTambah").length > 0) {
		$("#formTambah").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					url: "<?php echo base_url('administrator/operator/simpan') ?>",
					type: "POST",
	...
	$('#show_data').on('click', '.item_hapus', function() {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('administrator/operator/delete') ?>",
					async: true,
					dataType: "JSON",
					data: {
						id: id,
	...
	function show_data() {
		$.ajax({
			type: 'POST',
			url: '<?php echo site_url('administrator/operator/tampil') ?>',
			async: true,
			dataType: 'json',
			success: function(data) {
				var html = '';
	...
	$('#show_data').on('click', '.item_edit', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('administrator/operator/tampil_byid') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
