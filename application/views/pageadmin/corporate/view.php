<section class="content">
						...
						<div class="row">
							<div class="col-md-6">
								<div class="card-body">
									...
									no_services as maxno from customer_corporate order by id desc")->result_array();
									if (count($noreg) < 1) {
										$urutan = 'CRP00001';
									} else {
										$no = $noreg[0]['maxno'];
										...
										$urutan = $urutan + 1;
										$urutan = 'CRP' . sprintf("%05s", $urutan);
									}
									?>
									<div class="form-group">
										<label>ID Pelanggan</label>
										<input readonly value="<?= $urutan ?>" required type="text" id="nomor_layanan" name="nomor_layanan" class="form-control" placeholder="Nomor Layanan">
									</div>
	...
	<div id="modalPrev" class="modal fade" tabindex="-1">
		<div class="modal-dialog modal-xl">
			<div class="modal-content">
				<form class="form-horizontal" role="form" id="formPrev">
					<div class="card card-info">
						<div class="modal-header">
							<h4 class="modal-title">Preview Pelanggan</h4>
							<button type="button" class="close" data-dismiss="modal" aria-label="Close">
								<span aria-hidden="true">&times;</span>
							</button>
						</div>
						<div class="row">
							<div class="col-md-6">
								<div class="card-body">
									...
									<div class="form-group">
										<label>Nama Pelanggan</label>
										<input readonly required type="hidden" id="e_id2" name="e_id2">
									...
									<div class="form-group">
										<label>Kode ODC</label>
										<select readonly class="form-control select2" style="width: 100%;" name="e_kodc" id="e_kodc">
											<option selected="selected">-- Pilih --</option>
										</select>
									</div>
									...
									<div class="form-group">
										<label>Merek Perangkat</label>
										<select readonly class="form-control select2" style="width: 100%;" name="e_merekperangkat" id="e_merekperangkat">
											<option value="" selected="selected">-- Pilih --</option>
											<?php foreach ($mymerek as $value) { ?>
												<option value=<?= $value['id'] ?>><?= $value['nama'] ?></option>
											<?php } ?>
							...
							<div class="col-md-6">
								<div class="card-body">
									...
									<div class="form-group">
										<label>Tanggal Aktivasi</label>
										<input readonly type="date" id="e_tglaktivasi2" name="e_tglaktivasi2" class="form-control" placeholder="Tanggal Aktivasi">
									</div>

									<div class="form-group">
										<label>Jenis Tempat</label>
										<select readonly class="form-control select2" style="width: 100%;" name="e_jenistempat" id="e_jenistempat">
											<option value="" selected="selected">-- Pilih --</option>
											<?php foreach ($myjenistempat as $value) { ?>
												<option value=<?= $value['id'] ?>><?= $value['nama'] ?></option>
											<?php } ?>
...
<script type="text/javascript">
	if ($("#formTambah").length > 0) {
		$("#formTambah").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					url: "<?php echo base_url('administrator/corporate/simpan') ?>",
					type: "POST",
					data: formdata,
					processData: false,
					contentType: false,
	...
	$('#show_data').on('click', '.item_hapus', function() {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('administrator/corporate/delete') ?>",
					async: true,
					dataType: "JSON",
					data: {
						id: id,
	...
	function show_data() {
		$.ajax({
			type: 'POST',
			url: '<?php echo site_url('administrator/corporate/tampil') ?>',
			async: true,
			dataType: 'json',
			success: function(data) {
				var html = '';
	...
	$('#show_data').on('click', '.item_approve', function() {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('administrator/corporate/aktif') ?>",
					async: true,
					dataType: "JSON",
					data: {
						id: id,
	...
	$('#show_data').on('click', '.item_non', function() {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('administrator/corporate/nonaktif') ?>",
					async: true,
					dataType: "JSON",
					data: {
						id: id,
	...
	$('#show_data').on('click', '.item_edit', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('administrator/corporate/tampil_byid') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
	...
	if ($("#formEdit").length > 0) {
		...
		$("#formEdit").validate({
			...
			submitHandler: function(form) {
				...
				formdata = new FormData(form);
				$.ajax({
					url: "<?php echo base_url('administrator/corporate/update') ?>",
					type: "POST",
					data: formdata,
					processData: false,
					contentType: false,
	...
	function show_data_odc(id, callback) {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('administrator/corporate/showodc') ?>",
			data: {
				id: id
			}
		}).done(function(data) {
	...
	function show_data_eodc(id, callback) {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('administrator/corporate/showEditodc') ?>",
			data: {
				id: id
			}
		}).done(function(data) {
	...
	function show_data_eodp(id, callback) {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('administrator/corporate/showEditodp') ?>",
			data: {
				id: id
			}
		}).done(function(data) {
	...
	function show_data_odp(id, callback) {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('administrator/corporate/showodp') ?>",
			data: {
				id: id
			}
		}).done(function(data) {
