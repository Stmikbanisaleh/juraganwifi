...
<script type="text/javascript">
	if ($("#formTambah").length > 0) {
		$("#formTambah").validate({
			...
			submitHandler: function(form) {
				...
				$.ajax({
					url: "<?php echo base_url('administrator/services2/simpan') ?>",
					type: "POST",
					data: formdata,
					processData: false,
					contentType: false,
	...
	$('#show_data').on('click', '.item_hapus', function() {
		...
		Swal.fire({
			...
			if (result.value) {
				$.ajax({
					type: "POST",
					url: "<?php echo base_url('administrator/services2/delete') ?>",
					async: true,
					dataType: "JSON",
					data: {
						id: id,
	...
	function show_data() {
		$.ajax({
			type: 'POST',
			url: '<?php echo site_url('administrator/services2/tampil') ?>',
			async: true,
			dataType: 'json',
			success: function(data) {
				var html = '';
	...
	$('#show_data').on('click', '.item_edit', function() {
		...
		$.ajax({
			type: "POST",
			url: "<?php echo base_url('administrator/services2/tampil_byid') ?>",
			async: true,
			dataType: "JSON",
			data: {
				id: id,
	...
	if ($("#formEdit").length > 0) {
		$("#formEdit").validate({
			...
			submitHandler: function(form) {
				...
				formdata = new FormData(form);
				$.ajax({
					url: "<?php echo base_url('administrator/services2/update') ?>",
					type: "POST",
					data: formdata,
					processData: false,
					contentType: false,
