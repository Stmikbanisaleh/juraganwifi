...
class Model_daftar_tagihanc extends CI_model
{
    ...
    {
        return $this->db->query("select a.*, b.nominal_bayar as  Nominal_bayar  from invoice_corporate a join invoice_detail_corporate b on a.id = b.invoice_id
        where a.id = $data");
	}

    public function gettagihan($id)
    {
        return $this->db->query("select b.price from invoice_corporate a join invoice_detail_corporate b 
